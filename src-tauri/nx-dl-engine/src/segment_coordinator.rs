...
// 原始路径: native/engine/src/segment_coordinator.rs
...
/// 检查某域名是否被限制为单连接（且未过期）。
/// 仅测试使用：生产代码直接读取 [`domain_conn_cap`] 原值做并发上限决策。
#[cfg(test)]
...
pub async fn load_domain_conn_caps(db: &Db) {
...
pub fn clear_domain_conn_caps(db: &Db) {
    if let Ok(mut cache) = conn_cap_cache().lock() {
        if cache.is_empty() {
            return;
        }
...
/// 启动时的初始并发连接数（ramp-up 起点）。
/// NexBox 偏差（上游 2）：对齐 IDM 的体验——容忍型服务器不必从零爬坡，
/// 首个评估窗即可观察 4 连接的真实基线；惩罚型服务器由 collapse 回滚
/// （0.65 判据）+ 域名 cap 记忆兜底，起步即 4 的额外风险有限。
...
/// NexBox 偏差（上游 1.05）：国内大量服务器对每条连接施加令牌桶限速——
/// 新连接建立时的突发额度会让紧随其后的 2s 评估窗虚高，1.05 判据被
/// 突发噪声骗过、一路爬进惩罚区。提高到 1.25 要求"持续显著增益"
/// 才继续扩容，代价是甜点估计略保守（可接受：宁少勿罚）。
const RAMP_IMPROVE_FACTOR: f64 = 1.25;
...
/// 上游 0.5 面向"数量级级"惩罚（实测 59 MB/s -> 2 MB/s）；NexBox 偏差
/// （0.65）：国内服务器更常见的是温和的比例掐流（-30%~-50%），0.5 永远
/// 触不到回滚，只能 Freeze 在被惩罚规模。0.65 可捕获这类温和惩罚并回滚
/// + 记忆，误触发风险由 IMPROVE_FACTOR 同步收紧兜底。
const RAMP_COLLAPSE_FACTOR: f64 = 0.65;
...
pub async fn run_coordinated_download(
    ...
    // 慢启动宽限窗计数：扩容后的第一个评估窗被新连接 TCP slow-start/TLS
    // 握手拖累，直接拿它判定扩容效果会系统性低估、偏向冻结在低并发
    // （NexBox 偏差，上游无此机制——上游以 1.05 宽判据吸收该偏差，
    // NexBox 收紧到 1.25 后必须补宽限窗才能两全）。
    let mut ramp_warmup_ticks: u32 = 0;
        ...
        tokio::select! {
            ...
            _ = ramp_interval.tick() => {
                ...
                if !serial_mode && !all_done(&segments) && range_ok {
                    ...
                    // 2. 评估上一次扩容/试探的效果。
                    if awaiting_ramp_eval {
                        // 慢启动宽限窗：扩容后首个窗口被新连接 slow-start 拖累，
                        // 跳过采样与判定，保留 awaiting 状态让下个整窗干净测量。
                        if ramp_warmup_ticks > 0 {
                            ramp_warmup_ticks -= 1;
                        } else {
                        awaiting_ramp_eval = false;
                        ...
                        if was_probe {
                            ...
                            match verdict {
                                ...
                                RampVerdict::Collapse => {
                                    ...
                                    // 自然退休；进行中的连接与已下数据零丢弃。
                                    //
                                    // NexBox 偏差（上游无条件记忆）：域名 cap 持久化
                                    // 必须有【服务器拒绝类证据】佐证（403/429 拒绝
                                    // strike 或连接敏感信号）。纯吞吐崩塌不再写缓存——
                                    // 瓶颈链路上有其它下载器（IDM/浏览器/另一任务）
                                    // 争抢时，本任务吞吐同样会骤降，误记低值会让同
                                    // 域名此后 24h 全部以低并发起步（竞争性塌缩被
                                    // 当成惩罚性限速永久学习，越用越慢）。
                                    let rejection_backed =
                                        reject_strikes > 0 || conn_sensitive.load(Ordering::Relaxed);
                                    if rejection_backed {
                                        record_domain_conn_cap_persist(
                                            url,
                                            allowed_workers as i32,
...
mod tests {
    ...
    /// 恰在崩塌线上（等于 前值 × COLLAPSE_FACTOR）判 Freeze——回滚只留给明确信号。
    /// NexBox 偏差：COLLAPSE_FACTOR 由 0.5 收紧为 0.65，边界值随之移动；
    /// 断言改为以常量为基准的相对表达，避免再与具体数值耦合。
    #[test]
    fn ramp_verdict_boundary_is_freeze() {
        let pre = 10_000_000.0;
        let collapse_factor = super::RAMP_COLLAPSE_FACTOR;
        assert_eq!(
            ramp_verdict(pre, pre * collapse_factor),
            RampVerdict::Freeze
        );
        // 略低于崩塌线 → Collapse（温和比例掐流也应回滚）
        assert_eq!(
            ramp_verdict(pre, pre * (collapse_factor - 0.05)),
            RampVerdict::Collapse
        );
    }
