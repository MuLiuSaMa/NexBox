// JEP106 (JEDEC) 内存厂商识别码解码。参考 tubatool-master 移植：
// TubaWinUi3.WinUI3/Services/HardwareInfoService.cs + JedecVendorRegistry.cs
// 用于把 Win32_PhysicalMemory.Manufacturer 返回的十六进制 JEDEC 码或占位串
// 解析为真实内存品牌（金士顿、三星、美光、海力士、金百达、威刚、光威、阿斯加特等）。

/// JEP106BL 全表（bank 0-16）。key = (continuation/bank << 8) | vendor code，均为去掉奇校验位后的值。
/// 数据由 JedecVendorRegistry.cs 逐条转写，按 key 升序排列以供二分查找。
static JEP106_TABLE: &[(u16, &str)] = &[
    (0x0001, "AMD"),
    (0x0002, "AMI"),
    (0x0003, "Fairchild"),
    (0x0004, "富士通(Fujitsu)"),
...
/// 在按升序排列的 JEP106_TABLE 中二分查找厂商名。
fn lookup_registry(code: u16) -> Option<&'static str> {
    JEP106_TABLE
        .binary_search_by_key(&code, |&(c, _)| c)
        .ok()
        .map(|i| JEP106_TABLE[i].1)
}
...
/// JEP106 page 0（无 0x7F 续接字节）单字节裸厂商码短表。
/// 调用方需先去掉奇校验位。只收录明确的内存厂商；无法确证返回 None。
pub fn jedec_vendor_from_code(code: u8) -> Option<&'static str> {
    match code & 0x7F {
        0x04 => Some("富士通(Fujitsu)"),
        0x07 => Some("日立(Hitachi)"),
        0x08 => Some("Inmos"),
        0x0E => Some("飞思卡尔(Freescale/Motorola)"),
        0x10 => Some("NEC"),
        0x15 => Some("NXP(原飞利浦半导体)"),
        0x17 => Some("德州仪器(TI)"),
        0x18 => Some("东芝内存(Kioxia)"),
        0x1C => Some("三菱(Mitsubishi)"),
        0x1F => Some("Atmel"),
        0x20 => Some("意法半导体(ST)"),
        0x2C => Some("美光(Micron)"),
        0x2D => Some("海力士(SK Hynix)"),
        0x32 => Some("松下(Panasonic)"),
        0x40 => Some("茂德(ProMOS/Mosel)"),
        0x41 => Some("英飞凌(Infineon)"),
        0x4E => Some("三星(Samsung)"),
        0x55 => Some("ISSI"),
        0x5A => Some("华邦(Winbond)"),
        _ => None,
    }
}

/// 两字节打包码查全表。禁止回退单字节表（否则如 0x120B 会被误判成 Toshiba）。
fn jedec_vendor_from_extended_code(full_code: u16) -> Option<&'static str> {
    lookup_registry(full_code)
}

/// 解析一组 [bank][vendor] 打包值：高/低字节各去奇校验位后，先按大端再按小端查表。
fn decode_packed(code: u16) -> Option<&'static str> {
    let hi = ((code >> 8) & 0x7F) as u16;
    let lo = (code & 0x7F) as u16;
    let packed = (hi << 8) | lo;
    if let Some(v) = jedec_vendor_from_extended_code(packed) {
        return Some(v);
    }
    let swapped = (lo << 8) | hi;
    jedec_vendor_from_extended_code(swapped)
}

/// 把十六进制 JEDEC 码字符串解码为厂商名；非 hex 码返回 None。
pub fn decode_jedec_manufacturer(raw: &str) -> Option<&'static str> {
    ...
    // 2 位裸厂商码：去奇校验后走单字节表。
    if trimmed.len() == 2 && is_hex(trimmed) {
        if let Some(code) = parse_hex_u16(trimmed) {
            return jedec_vendor_from_code(code as u8);
        }
        return None;
    }

    // 4 位：[bank][vendor] 双字节，兼容大端/小端两种字节序。
    if trimmed.len() == 4 && is_hex(trimmed) {
        if let Some(code) = parse_hex_u16(trimmed) {
            return decode_packed(code);
        }
        return None;
    }

    // 0x 前缀形式（CPU-Z / 部分 BIOS）。
    let upper = trimmed.to_ascii_uppercase();
    if upper.len() >= 4 && upper.starts_with("0X") {
        let hex_part = &upper[2..];
        if hex_part.len() == 2 && is_hex(hex_part) {
            if let Some(code) = parse_hex_u16(hex_part) {
                return jedec_vendor_from_code(code as u8);
            }
        } else if hex_part.len() == 4 && is_hex(hex_part) {
            if let Some(code) = parse_hex_u16(hex_part) {
                return decode_packed(code);
            }
        }
...
/// 归一化内存厂商：解码 hex / 映射字符串品牌 / 过滤占位串。
/// 返回 None 表示无有效厂商信息（占位串、空或未知品牌）。
pub fn clean_mem_manufacturer(raw: Option<&str>) -> Option<&'static str> {
    let raw = raw?;
    ...
    // 1) 优先按十六进制 JEDEC 全表解码。
    if let Some(decoded) = decode_jedec_manufacturer(cleaned) {
        return Some(decoded);
    }

    // 2) 字符串品牌映射（统一输出“中文(English)”格式）与占位串过滤。
    match cleaned.to_ascii_uppercase().as_str() {
        "UNKNOWN" | "TO BE FILLED BY O.E.M." | "TO BE FILLED BY OEM" | "DEFAULT STRING"
        | "NOT SPECIFIED" | "NOT AVAILABLE" | "N/A" | "NONE" | "UNDEFINED" => None,
        "KINGSTON" | "KINGSTON TECHNOLOGY" => Some("金士顿(Kingston)"),
        "CORSAIR" => Some("海盗船(Corsair)"),
        "CRUCIAL" | "CRUCIAL TECHNOLOGY" => Some("英睿达(Crucial)"),
        "SAMSUNG" | "SAMSUNG ELECTRONICS" => Some("三星(Samsung)"),
        "SK HYNIX" | "HYNIX" => Some("海力士(SK Hynix)"),
        "MICRON" | "MICRON TECHNOLOGY" => Some("美光(Micron)"),
        "ADATA" | "ADATA TECHNOLOGY" | "A-DATA" | "A-DATA TECHNOLOGY" => Some("威刚(ADATA)"),
        "G.SKILL" | "GSKILL" => Some("芝奇(G.Skill)"),
        "TEAM" | "TEAMGROUP" | "TEAM GROUP" => Some("十铨(TeamGroup)"),
        "GEIL" => Some("金邦(Geil)"),
        "APACER" => Some("宇瞻(Apacer)"),
        "PATRIOT" => Some("博帝(Patriot)"),
        "SILICON POWER" | "S-POWER" | "SP" => Some("广颖电通(Silicon Power)"),
        "KLEVV" => Some("科赋(Klevv)"),
        "BIWIN" => Some("佰维(Biwin)"),
        "GALAX" | "GALAXY" => Some("影驰(Galax)"),
        "COLORFUL" => Some("七彩虹(Colorful)"),
        "LONGSYS" => Some("江波龙(Longsys)"),
        "NETAC" => Some("朗科(Netac)"),
        "PNY" => Some("必恩威(PNY)"),
        "GOODRAM" => Some("Goodram"),
        "RAMAXEL" => Some("记忆科技(Ramaxel)"),
        "CXMT" => Some("长鑫存储(CXMT)"),
        "KINGBANK" | "KINGBANK TECHNOLOGY" => Some("金百达(Kingbank)"),
        "KINGMAX" | "KINGMAX TECHNOLOGY" | "KINGMAX SEMICONDUCTOR" => Some("胜创(Kingmax)"),
        "ASINT" => Some("ASint"),
        "V-COLOR" | "VCOLOR" => Some("V-Color"),
        "GLOWAY" => Some("光威(Gloway)"),
        "ASGARD" => Some("阿斯加特(Asgard)"),
        "JUHOR" => Some("玖合(JUHOR)"),
        "TECLAST" => Some("台电(Teclast)"),
        "MAXSUN" => Some("铭瑄(Maxsun)"),
        "KIMTIGO" => Some("金泰克(Kimtigo)"),
        "AIGO" => Some("爱国者(aigo)"),
        "AVEXIR" => Some("宇帷(Avexir)"),
        "TWINMOS" => Some("勤茂(TwinMOS)"),
        "NEO FORZA" | "NEOFORZA" => Some("凌航(Neo Forza)"),
        "ESSENCORE" => Some("科赋(Essencore)"),
        _ => None,
    }
}

/// 最终解析：已知品牌用中文名；无法识别的真实品牌保留原文；占位/空返回 None。
pub fn resolve_mem_manufacturer(raw: Option<&str>) -> Option<String> {
    if let Some(known) = clean_mem_manufacturer(raw) {
    ...
    // clean 已过滤占位串；此处仍可能剩下未收录的真实厂商名，二次排除纯占位。
    match cleaned.to_ascii_uppercase().as_str() {
        "UNKNOWN" | "TO BE FILLED BY O.E.M." | "TO BE FILLED BY OEM" | "DEFAULT STRING"
        | "NOT SPECIFIED" | "NOT AVAILABLE" | "N/A" | "NONE" | "UNDEFINED" => None,
...
mod tests {
    ...
    #[test]
    fn placeholder_returns_none() {
        assert_eq!(clean_mem_manufacturer(Some("Unknown")), None);
        assert_eq!(clean_mem_manufacturer(Some("To be filled by O.E.M.")), None);
        assert_eq!(clean_mem_manufacturer(Some("Default string")), None);
        assert_eq!(clean_mem_manufacturer(Some("Not Specified")), None);
        assert_eq!(clean_mem_manufacturer(Some("N/A")), None);
        assert_eq!(clean_mem_manufacturer(Some("")), None);
        assert_eq!(clean_mem_manufacturer(Some("   ")), None);
        assert_eq!(clean_mem_manufacturer(None), None);
    }

    #[test]
    fn string_brand_maps_to_chinese() {
        assert_eq!(clean_mem_manufacturer(Some("KINGSTON")), Some("金士顿(Kingston)"));
        assert_eq!(clean_mem_manufacturer(Some("Kingston Technology")), Some("金士顿(Kingston)"));
        assert_eq!(clean_mem_manufacturer(Some("A-DATA Technology")), Some("威刚(ADATA)"));
        assert_eq!(clean_mem_manufacturer(Some("CORSAIR")), Some("海盗船(Corsair)"));
        assert_eq!(clean_mem_manufacturer(Some("CRUCIAL")), Some("英睿达(Crucial)"));
    }

    #[test]
    fn single_byte_hex() {
        assert_eq!(clean_mem_manufacturer(Some("2C")), Some("美光(Micron)"));
        assert_eq!(clean_mem_manufacturer(Some("CE")), Some("三星(Samsung)"));
        assert_eq!(clean_mem_manufacturer(Some("18")), Some("东芝内存(Kioxia)"));
        assert_eq!(decode_jedec_manufacturer("0x2C"), Some("美光(Micron)"));
        assert_eq!(decode_jedec_manufacturer("0xCE"), Some("三星(Samsung)"));
    }

    #[test]
    fn four_byte_dual_endianness_and_parity() {
        assert_eq!(clean_mem_manufacturer(Some("0B12")), Some("金百达(Kingbank)"));
        assert_eq!(clean_mem_manufacturer(Some("120B")), Some("金百达(Kingbank)"));
        assert_eq!(clean_mem_manufacturer(Some("8B92")), Some("金百达(Kingbank)"));
        assert_eq!(clean_mem_manufacturer(Some("928B")), Some("金百达(Kingbank)"));
        assert_eq!(clean_mem_manufacturer(Some("00CE")), Some("三星(Samsung)"));
        assert_eq!(clean_mem_manufacturer(Some("CE00")), Some("三星(Samsung)"));
        assert_eq!(clean_mem_manufacturer(Some("0118")), Some("金士顿(Kingston)"));
        assert_eq!(clean_mem_manufacturer(Some("0198")), Some("金士顿(Kingston)"));
        assert_eq!(clean_mem_manufacturer(Some("051B")), Some("英睿达(Crucial)"));
        assert_eq!(clean_mem_manufacturer(Some("0746")), Some("光威(Gloway)"));
        assert_eq!(clean_mem_manufacturer(Some("0813")), Some("光威(Gloway)"));
        assert_eq!(clean_mem_manufacturer(Some("0871")), Some("阿斯加特(Asgard)"));
        assert_eq!(clean_mem_manufacturer(Some("0B2E")), Some("爱国者(aigo)"));
        assert_eq!(clean_mem_manufacturer(Some("0x120B")), Some("金百达(Kingbank)"));
        assert_eq!(clean_mem_manufacturer(Some("1003")), Some("JoulWatt Technology Co Ltd"));
    }
}
